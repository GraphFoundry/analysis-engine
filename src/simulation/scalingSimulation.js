...
async function simulateScaling(request, options = {}) {
    ...
    // Overlay historical telemetry if requested
    if (timeWindow) {
        try {
            const { from, to } = telemetryService.parseTimeWindow(timeWindow);
            const aggregatedMetrics = await telemetryService.getAggregatedEdgeMetrics(from, to);

            if (aggregatedMetrics.size > 0) {
                // Update edges in the snapshot
                snapshot.edges.forEach(edge => {
                    const key = `${edge.source}->${edge.target}`;
                    const metrics = aggregatedMetrics.get(key);
                    if (metrics) {
                        edge.rate = metrics.requestRate;
                        edge.error_rate = metrics.errorRate;
                        edge.p50 = metrics.p50;
                        edge.p95 = metrics.p95;
                        edge.p99 = metrics.p99;
                    } else {
                        // If no historical data for this edge, assume zero traffic for simulation consistency
                        edge.rate = 0;
                        edge.error_rate = 0;
                        edge.p50 = 0;
                        edge.p95 = 0;
                ...
                trace.log('Overlayed historical metrics', { timeWindow, metricsCount: aggregatedMetrics.size });
            }
        } catch (err) {
            console.error('Failed to overlay telemetry in simulateScaling:', err);
